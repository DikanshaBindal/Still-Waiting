...
import { saveToCache, loadFromCache, shortenAddress, calculateTotal } from './utils.js';
...
// Wait for DOM to load before setting contract address & initializing cache
document.addEventListener("DOMContentLoaded", () => {
    const uiContract = document.getElementById('ui-contract-address');
    if (uiContract) {
        uiContract.textContent = shortenAddress(RECEIPT_CONTRACT_ID);
    }
    initApp();
});

const initApp = () => {
    const cachedWallet = loadFromCache('wallet');
    const history = loadFromCache('history') || [];
    
    if (cachedWallet) {
        showError(`Previously connected wallet detected: ${shortenAddress(cachedWallet)}`);
    }
...
const renderTransactionHistory = (history) => {
    const section = document.getElementById('recent-transactions-section');
        ...
        list.innerHTML = history.map(tx => `
            <div class="card" style="padding: 10px; font-size: 0.8rem; background: rgba(255,255,255,0.03);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    <span style="color: var(--accent-color); font-family: monospace;">${shortenAddress(tx.hash)}</span>
...
window.connectWallet = async function() {
    try {
        ...
        saveToCache('wallet', userPublicKey);
...
window.processPayment = async function() {
    ...
    try {
        ...
        titleText.textContent = "Waiting for Signature";
        ...
        try {
            ...
            // Extract real Stellar error details from the Horizon response
            let horizonMsg = "network failure";
            if (e.response && e.response.data) {
                ...
                if (extras && extras.result_codes) {
                    ...
                    horizonMsg = `horizon: ${codes.transaction || e.response.data.title}${opCodes ? ' | op: ' + opCodes : ''}`;
        ...
        document.getElementById('receipt-wallet-addr').textContent = shortenAddress(userPublicKey);
        const history = loadFromCache('history') || [];
        const newTx = { hash: result.hash, amount: amountToCharge, date: Date.now() };
        const updatedHistory = [newTx, ...history].slice(0, 3);
